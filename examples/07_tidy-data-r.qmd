...
Whoops. More than 20 unmatched rows. Which locations didn't get a match? We can filter to look for `NA` values and then use `distinct()` on the join column.
...
new_units_wide %>% 
  mutate(
    county = stringr::str_remove(county, "County$|city$")
  ) %>% 
ggplot() +
...
🤔 Can you convert the data into a long format with `pivot_long()`?

```{r}
# new_multifamily_units <- pivot_longer()
```
...
permits <-
  ...
  bbox = mapbaltimore::neighborhoods[63,]
)
```
