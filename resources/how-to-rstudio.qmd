...
Creating a README with RMarkdown allows you to include executable code chunks in your README. You should render your README as a Markdown file with `knitr::knit("README.Rmd")` (or `devtools::build_readme()` if your project is an R package) any time you make changes.
:::
...
-   Install the styler package and then use the Addins drop-down in RStudio to run the Style active file command (or run `styler:::style_active_file()` in the Console pane).
...
-   Code with shorter lines is easier to read and understand. When writing a function call with a large number of parameters, put each parameter on a separate line.
...
-   Always have a space between a heading tag, e.g. `##` and the heading text.
