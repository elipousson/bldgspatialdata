...
## Chaining functions with pipes: `%>%` or `|>`

Pipes are helpful for creating multi-step data transformation workflows that are easier to read and understand:
...
## Bonus: Maps and charts with {ggplot2}
...
The use of pipes is especially helpful with data visualizations where it reduces the need for intermediate objects or exports.
...
# Attribute data with QGIS



------------------------------------------------------------------------

But you can drop geometry if you need to (you can add it back later):

```{r}
us_states <- sf::st_drop_geometry(us_states)
```

------------------------------------------------------------------------

You can pass filtered data frames to other functions, such as `nrow()`, `length()`, `unique()`, or `dplyr::count()`:

```{r}
# How many U.S. states lost population from 2010 to 2015?
us_states[(us_states$total_pop_15 - us_states$total_pop_10) < 0, ] %>% 
  nrow()
```
...
```{r}
# Subset rows 10 to 12
slice(us_states, c(10:12))

# Subset first 2 rows
slice_head(us_states, n = 2)
```
